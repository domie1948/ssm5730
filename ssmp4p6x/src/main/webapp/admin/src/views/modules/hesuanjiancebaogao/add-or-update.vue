          ...
          <el-form-item class="input" v-if="ruleForm.shangchuanshijian" label="上传时间" prop="shangchuanshijian">
              <el-input v-model="ruleForm.shangchuanshijian" 
                placeholder="上传时间" readonly></el-input>
                                                                                                                                                                                                      ...
                                                                                                                                                                                                      <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="报告" prop="baogao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.baogao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.baogao" label="报告" prop="baogao">
                    <span v-html="ruleForm.baogao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
                                                                        ...
                                                                        // 多级联动参数
                                                                                            info(id) {
      this.$http({
        url: `hesuanjiancebaogao/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `hesuanjiancebaogao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.hesuanjiancebaogaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
